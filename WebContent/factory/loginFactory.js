app.factory('loginFactory', [
		...
		function($cookies, $http, $state, $rootScope, $q, $window) {
			...
			loginfac.signDetails = function(login) {
				...
				return $http.post('http://18.188.172.194:8000/domo/login/',
						login).then(
						function(response) {
			...
			loginfac.contactus = function(query) {
				return $http.post('http://18.188.172.194:8000/domo/getInTouch/', query).then(function(response) {
					return response.data;
				}, function(errResponse) {
					return errResponse.data;
				});
			...
			loginfac.secQuestion = function(secQuestion) {
				...
				return $http.post(
						'http://18.188.172.194:8000/domo/secQuestion/',
			...
			loginfac.updatePassword = function(newPassword) {
				...
				return $http.post(
						'http://18.188.172.194:8000/domo/updatePassword/',
