				...
				[
						...
						function($state, $scope, $window, $http, $cookies,
							...
							displayConfiguration = function() {
								...
								if ($window.location.pathname == '/admin/byd-domo.html') {
									userObj["service_type"] = 'SAPBYD';
								} else if ($window.location.pathname == '/admin/c4c-domo.html') {
									userObj["service_type"] = 'SAPC4C';
								}
								else if ($window.location.pathname == '/admin/bw-domo.html'){
									userObj["service_type"] = 'SAPBW';
								}
							...
							/*
							 * readSapC4C = function() { var runObj = 'SAPC4C';
							 * userObj["service_type"] = runObj;
							 * 
							 * configurationFactory .readConfiguration(userObj)
							 * .then( function(response) {
							 * 
							 * $scope.responseSAPC4C = [ response ];
							 * console.log($scope.responseSAPC4C);
							 * $scope.errorConfig = response; var arr = Object
							 * .values($scope.errorConfig); str1 = arr.join("");
							 * var substringResult = str1 .substring(0, 32);
							 * $scope.errorConfiguration = substringResult;
							 * console .log($scope.errorConfiguration); },
							 * function(error) { $scope.status = 'Error
							 * retrieving customers! ' + error.message; }) }
							 * 
							 * readSapC4C();
							 */
							...
							$scope.createConfig = function(configuration,
									config) {

								configuration["service_type"] = config;
