app.factory('configurationFactory', [
		...
		function($http) {
			...
			configurationfac.readConfiguration = function(config) {
				...
				return $http.post('http://18.188.172.194:8989/domo/readConfig/',
			...
			configurationfac.viewLogs = function(config) {
				...
				return $http.post(
						'http://18.188.172.194:8989/domo/readUserLogs/', body)
						.then(function(response) {
							//alert(response.data + "Success");
							return response.data;
						}, function(errResponse) {
			...
			configurationfac.disableConfig = function(config) {
				var body = JSON.stringify(config);
				return $http.post(
						'http://18.188.172.194:8989/domo/disableConfig/', body)
						.then(function(response) {
							return response.data;
						}, function(errResponse) {
							return errResponse.data;
			...
			configurationfac.runConfiguration = function(config) {
				var body = JSON.stringify(config);
				return $http.post(
						'http://18.188.172.194:8989/domo/domo_upload/', body)
						.then(function(response) {
							return response.data;
						}, function(errResponse) {
							return errResponse.data;
